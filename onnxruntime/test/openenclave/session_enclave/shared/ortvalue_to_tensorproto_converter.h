...
common::Status MLValueToTensorProto(const OrtValue& ml_value, bool using_raw_data,
  ...
  switch (data_type) {
    case onnx::TensorProto_DataType_FLOAT: {  // Target: raw_data or float_data
      ...
      if (using_raw_data) {
        tensor_proto.set_raw_data(data, tensor.SizeInBytes());
      } else {
    ...
    case onnx::TensorProto_DataType_UINT8: {  // Target: raw_data or int32_data
      ...
      if (using_raw_data) {
        ...
        for (int i = 0, count = 1 + ((tensor.SizeInBytes() - 1) / sizeof(int32_t)); i < count; ++i) {
    ...
    case onnx::TensorProto_DataType_UINT32: {  // Target: raw_data or uint64_data
      ...
      if (using_raw_data) {
        ...
        for (int i = 0, count = 1 + ((tensor.SizeInBytes() - 1) / sizeof(uint64_t)); i < count; ++i) {
...
Status OrtValueToTensorProto(const OrtValue* value, void** tensor_proto_buf, size_t* tensor_proto_size) {
  if (!value->IsTensor()) {
    return common::Status(common::StatusCategory::ONNXRUNTIME,
  ...
  ORT_RETURN_IF_ERROR(MLValueToTensorProto(*value, true, tensor_proto));
  *tensor_proto_size = tensor_proto.ByteSizeLong();
  *tensor_proto_buf = malloc(*tensor_proto_size);
  if (!tensor_proto.SerializeToArray(*tensor_proto_buf, *tensor_proto_size)) {
    return common::Status(common::StatusCategory::ONNXRUNTIME,
                          common::StatusCode::RUNTIME_EXCEPTION,
                          "TensorProto serialization failed");
  }
  return common::Status::OK();
}

OrtStatus* OrtValueToTensorProto_C(const OrtValue* value, void** tensor_proto_buf, size_t* tensor_proto_size) {
  return ToOrtStatus(OrtValueToTensorProto(value, tensor_proto_buf, tensor_proto_size));
}
